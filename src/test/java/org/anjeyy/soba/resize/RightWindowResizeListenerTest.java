...
class RightWindowResizeListenerTest {
    ...
    private static void createResponsiveWindow(Stage stage) {
        ...
        AbstractWindowResizeListener.relevantMouseEvents()
                                    .forEach(m -> linkedScene.addEventHandler(m, rightWindowResizeListener));
    }

    @Order(1)
    @Test
    void resizing_right_window_border_to_right_is_successful(FxRobot fxRobot) throws TimeoutException {
        // given
        ...
        double xClick = originalX + originalWidth - 1;
        ...
        Assertions.assertThat(widthResized).isEqualTo(originalWidth + xResizeMovement - 1);
    ...
    @Order(2)
    @Test
    void resizing_right_window_border_to_left_is_successful(FxRobot fxRobot)
        ...
        double originalHeight = currentStage.getHeight();
        double xClick = originalX + originalWidth - 1;
        double yClick = (originalY + originalHeight) / 2 + 10;

        // when
        fxRobot.moveTo(0, 0)
               .moveTo(xClick + 100, yClick)
               .moveTo(xClick, yClick)
               .press(MouseButton.PRIMARY)
               .drag(xClick - xResizeMovement, yClick)
               .drop();
